...
- `sym3` to integrator typu RATTLE (Verlet prędkościowy z więzami rozwiązywanymi wewnątrz kroku): rząd 2, więzy na poziomie maszynowym, energia o rzędy wielkości dokładniejsza niż `sym2` przy tym samym koszcie, więc można użyć znacznie większego kroku. Przy sztywnym tłumieniu (`c/m·dt/2 > 1`) jawny `sym3` traci stabilność; tryb `sym3(..., polniejawne=True)` liczy krok prędkości półniejawnie (jeden krok Newtona z jakobianem `da/dv`) i jest wtedy bezwarunkowo stabilny w tłumieniu. Kosztuje to `7N` ewaluacji sił na krok, więc opłaca się dla małego i średniego `N`; dla pełnej sylwetki z kontaktem szybszy pozostaje `sym2`.
